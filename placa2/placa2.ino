...
void isrCAN()
{
  ...
  if (CAN.rxInterrupt()) {
    ...
    switch(rx_id){
      ...
      Serial.println("simulador cambio piso");
...
void TaskControl(){
    ...
    while(1)
    {
      so.waitFlag(fControl, maskCANEvent);
      flagValue = so.readFlag(fControl);
      so.clearFlag(fControl, maskCANEvent);
